...
export const getPurchasesPaginatedService = async (
  params: FilterParams
...
export const getTotalPurchasesInPeriodService = async (
  ...
  const response = await api.post<TotalPushchasesInPeriodResponse>(
    '/purchases/totals/period',
    payload
  );
  return response.data;
};
...
export const searchPurchasesByPeriodService = async (
  ...
  try {
    const response = await api.get<Purchase[]>(
      `/purchases/search/by-period?startDate=${startDate}&endDate=${endDate}`
    );
    return response.data;
  } catch (error) {
...
export const getPurchasesBySupplierService = async (
  supplierId: number
): Promise<PurchaseListResponse> => {
  const response = await api.get<PurchaseListResponse>(
    `/purchases/supplier?supplierId=${supplierId}`
  );
  return response.data;
};

export const getPurchasesByProductService = async (
  productId: number
): Promise<PurchaseListResponse> => {
