...
export const searchSalesByPeriodService = async (
  startDate: string,
  endDate: string
): Promise<SaleListResponse> => {
  try {
    const response = await api.get<SaleListResponse>(
      `/sales/period?startDate=${startDate}&endDate=${endDate}`
    );
    return response.data;
  } catch (error) {
...
export const updateSaleStatusService = async (
  saleId: number,
