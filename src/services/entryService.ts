...
export const getEntryPaginatedService = async (
  params: EntryPaginatedParams
...
export const searchExpensesByPeriodService = async (
  startDate: string,
  endDate: string
): Promise<Entry[]> => {
  try {
    const response = await api.get<Entry[]>(
      `/expenses/search/by-period?startDate=${startDate}&endDate=${endDate}`
    );
    return response.data;
  } catch (error) {
...
// export const getEntryRelatoryByMonthService = async (
//   params: EntryRelatoryRequestByMonth
// ): Promise<Blob> => {
...
export const getEntryRelatoryByPeriodService = async (
  params: EntryRelatoryRequestByPeriod
): Promise<Blob> => {
